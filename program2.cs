...
public class PingPong : MonoBehaviour
{
    ...
    void Update()
    {
        Move(gameObject,startpos,endpos,4f,true);
    }
    void Move(GameObject gameObject, Vector3 begin, Vector3 end, float time, bool pingpong)
    {
        ...
        if(!pingpong)
        {
            gameObject.transform.position = Vector3.Lerp(gameObject.transform.position, end, step);
        }
        if(pingpong)
        {
            if(!flag)
            ...
            {
                gameObject.transform.position = Vector3.Lerp(gameObject.transform.position,begin, step);
            }
